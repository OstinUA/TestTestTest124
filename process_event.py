...
# Получаем список разрешенных пользователей и приводим их к нижнему регистру для надежности
...
if event_name == "push":
    ...
    # Пропускаем коммиты слияния (merge commits)
...
def was_already_closed(title_keyword: str) -> bool:
    """Проверяет, не закрывали ли мы уже похожую задачу ранее."""
...
def build_permalink(filename: str, line: int = 1) -> str:
    """Создает прямую ссылку на конкретную строку кода в GitHub."""
...
# Определяем промпт в зависимости от найденных меток (labels)
...
def call_gemini(prompt: str, retries: int = 4, base_delay: int = 15) -> dict:
    """Отправляет запрос к Gemini API и безопасно обрабатывает ответ."""
    ...
    for model in models_to_try:
        ...
        for attempt in range(retries):
            try:
                ...
                # Убираем возможную Markdown-разметку, которую ИИ иногда добавляет
...
# Проверяем, не было ли это уже закрыто
...
# Подставляем правильную ссылку на код
if affected_file:
...
# Формируем подвал (footer) сообщения
if event_name == "push":
...
# Если это Pull Request, то оставляем еще и комментарий к нему
if pr_ref:
